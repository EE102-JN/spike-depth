...
import math
from .monocular_way import UNet, UNet_P, UNet_KD, UNet_KD1, UNet_KDD, UNet_KD_unc, UNet_KD_drop
...
class cfnet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        #self.mono_way = DORN(pretrained = False)
        #self.mono_way = MonoRes()
        #self.mono_way = UNet_KD_drop(n_channels = 32, n_classes = 1)
        #self.mono_way = UNet_KD_unc(n_channels = 32, n_classes = 2)
        self.mono_way = UNet_KD1(n_channels = 32, n_classes = 1)
        
        #self.mono_way = MonocularWay()
        self.norm = Generator()
        self.norm.load_state_dict(torch.load("/home/lijianing/snn/spikling-master/checkpoint/spikling-0100.pth"))
        
        #self.fusionet = fcn()
        #self.fusionet = UNet_P(n_channels = 1, n_classes = 1)
        ...
        #self.feature_extraction_sia = self.feature_extraction
        ...
        self.batch_size = 4
        #self.mencoder = ResnetEncoder(num_layers = 50, pretrained="scratch")
        
        #self.mdecoder = DepthDecoder(self.mencoder.num_ch_enc, scales = [0, 1, 2, 3])         
    ...
    def forward(self, left, right):
        ...
        #mono_features = self.bottle(mono_features["feature_last"])
        
        mono_depth = self.mono_up1(features_right["feature_last"])  
      
        mono_depth = self.mono_up2(mono_depth)  
 


        mono_depth = self.mono_way(mono_depth)#####["monocular"]
        ...
        if self.training:
            ...
            '''
            pred1_s2_4fu = torch.unsqueeze(pred1_s2, 1)
            pred1_s2_4fu = 1 / 256.0*pred1_s2_4fu 
            mono_depth_4fu = torch.unsqueeze(mono_depth, 1)
            doul_fusion = self.fusionet(pred1_s2_4fu, mono_depth_4fu)
            doul_fusion = F.upsample(doul_fusion, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            doul_fusion = torch.squeeze(doul_fusion, 1)
            '''
            result={}
            result['stereo'] = [pred0_4, pred1_4, pred2_s4, pred0_s3, predmid_s3, pred1_s3_up, pred0_s2, predmid_s2, pred1_s2]
            result['monocular'] = mono_depth####prob, label
            #result['fusion'] = doul_fusion
      
            return result#[pred0_4, pred1_4, pred2_s4, pred0_s3, predmid_s3, pred1_s3_up, pred0_s2, predmid_s2, pred1_s2]#
            ...
            ####mono_depth = torch.unsqueeze(mono_depth, 1)

            pred_depth = mono_depth
            '''
            pred1_s2_4fu = torch.unsqueeze(pred1_s2, 1)
            pred1_s2_4fu = 1 / (256*pred1_s2)
            mono_depth_4fu = torch.unsqueeze(mono_depth, 1)
            doul_fusion = self.fusionet(pred1_s2_4fu, mono_depth_4fu)
            doul_fusion = F.upsample(doul_fusion, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            doul_fusion = torch.squeeze(doul_fusion, 1)
            '''
            result={}
            result['stereo'] = [pred1_s2, pred1_s3_up, pred2_s4]
            result['monocular'] = pred_depth#ord_prob, ord_label#pred_depth,_,_,_,_
