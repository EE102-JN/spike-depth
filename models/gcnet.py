...
class GCNet(nn.Module):
    def __init__(self, batch_size, cost_volume_method, maxdisp):
        ...
        self.downsampling = nn.Sequential(
            nn.Conv2d(32, 32, 5, stride=2, padding=2), #1 origin
    ...
    def forward_stage2(self, feature_l, feature_r):
        ...
        cost_v_r = CostVolume(feature_r, feature_l, "right", method=self.cost_volume_method, k=4, batch_size=self.batch_size)
        
        #print(cost_v_l.size(), cost_v_r.size())
        cost_v_l = torch.cat((cost_v_l, cost_v_r), dim=1)
    ...
    def forward(self, left, right):
        ...
        d_final_l = F.upsample(d_final_l, [256,512], mode='bilinear', align_corners=True).squeeze(1)
        
        
        result = {}
        result["stereo"] = [d_final_l]
...
class MetricBlock(nn.Module):
    def __init__(self, in_channel, out_channel, stride = 1):
        ...
        self.relu = Mish()#nn.LeakyReLU(inplace=True)
...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, dilation=1, stride=1, downsample=None):
        ...
        self.relu1 = Mish()#nn.LeakyReLU(negative_slope=0.2, inplace=True)

        self.conv2 = nn.Conv2d(out_channel, out_channel, kernel_size=3, stride=stride,
                               padding=padding, dilation=dilation, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channel)
        self.relu2 = Mish()#nn.LeakyReLU(negative_slope=0.2, inplace=True)
...
def CostVolume(input_feature, candidate_feature, position="left", method="subtract", k=4, batch_size=2, channel=32, D=32, H=256, W=512):
    """
    Some parameters:
        position
