...
'''
with torch.cuda.device([6,7]):
    model = nn.DataParallel(model, [6,7]) 
'''
...
'''
kwargs={'map_location':lambda storage, loc: storage.cuda(6)}
...
model = load_GPU(model, '/home/lijianing/depth/MMlogs/256/cfnet/checkpoint_max_cfnetunet_uncertain3.ckpt', kwargs)
'''
...
def test_spike():
    ...
    model.load_state_dict(state_dict['model'])
    '''
    state_dict = torch.load('/home/lijianing/depth/MMlogs/256/cfnet/checkpoint_max_fus_unc_3.30.ckpt')#.module.state_dict()##/home/lijianing/depth/MMlogs/256/psmnet/checkpoint_max_ster16_3.29.ckpt
    model.load_state_dict(state_dict['model'])
    ...
    for sample in tqdm(TestImgLoader):
        ...
        disp_ests = ests['stereo']##['stereo']

        ###depth_ests,_,_,_,_ = ests['monocular']['monocular']
        ####depth_ests = ests['monocular']['depth']
        
        #disp_ests = depth_ests
        #depth_ests = disp_ests[-1]
                
        pred_depth = depth_ests
        disp_depth = 1/(disp_ests[-1])
        #print(disp_depth)
        
        #pred_depth = (1.2*128.0*(depth_ests + 1) + 0.8*disp_depth) / 2.0
        
        pred_disp = np.array(disp_ests[-1].detach().cpu(), dtype = np.float32).squeeze(0)
        disp_ests = np.array(disp_depth.detach().cpu(), dtype = np.float32).squeeze(0)
        ...
        #abs_rel_, sq_rel_, rmse_, rmse_log_, a1_, a2_ = compute_errors(disp_gt, pred_disp)
...
def test_dropout():
    ...
    model.load_state_dict(state_dict['model'])
    '''
    state_dict = torch.load('/home/lijianing/depth/MMlogs/256/cfnet/checkpoint_max_fusd_3.30.ckpt')#.module.state_dict()##/home/lijianing/depth/MMlogs/256/psmnet/checkpoint_max_ster16_3.29.ckpt
